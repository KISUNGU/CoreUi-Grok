<template>
  <CCard>
    <CCardHeader>
      <h2>Suivi des Courriers Entrants</h2>
    </CCardHeader>
    <CCardBody>
      ...
      <CTable v-else :items="incomingMails" :columns="fields" striped hover>
        ...
        <template #actions="{ item }">
          <td>
            <CButton color="primary" size="sm" @click="updateStatus(item.id, 'suivi')">Suivi</CButton>
          </td>
        </template>
      </CTable>
    </CCardBody>
  ...
  setup() {
    ...
    return {
      ...
      getBadgeColor(status) {
        return status === 'nouveau' ? 'success' : status === 'suivi' ? 'info' : 'secondary';
      },
    };
  },
  data() {
