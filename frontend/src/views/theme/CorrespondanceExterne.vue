<template>
  <CRow class="flex-column">
    ...
    <CCol xs="12">
      <CCard class="shadow-sm border-0 outlook-card">
        <CCardHeader class="outlook-header">
          <strong><i class="cil-list me-2"></i> Liste des Correspondances Externes</strong>
        </CCardHeader>
        <CCardBody>
          ...
          <table class="table table-striped table-hover custom-table">
            ...
            <tbody>
              ...
              <tr v-if="paginatedCorrespondances.length === 0">
                <td colspan="5" class="text-center">Aucune correspondance externe enregistrée</td>
              </tr>
            </tbody>
          </table>
          <!-- Pagination -->
    ...
    <CModal :visible="showAddModal" title="Ajouter une Correspondance Externe" @close="showAddModal = false" class="custom-modal">
      <CRow>
        ...
        <!-- Colonne droite : Date, Pièce jointe -->
        <CCol :xs="6">
          <div class="mb-3">
      ...
      <template #footer>
        ...
        <button
          ...
          :disabled="!correspondance.reference || !correspondance.destinataire || !correspondance.objet || !correspondance.date"
        >
          <i class="cil-check me-1"></i> Valider et Enregistrer
        </button>
      </template>
    </CModal>
...
export default {
  ...
  methods: {
    async fetchCorrespondances() {
      console.log('Récupération des correspondances externes...');
      try {
        const response = await fetch('http://localhost:3000/api/correspondances-externes');
        if (!response.ok) {
    ...
    validateForm() {
      let errors = [];
      if (!this.correspondance.reference) errors.push("Référence requise");
      if (!this.correspondance.destinataire) errors.push("Destinataire requis");
      if (!this.correspondance.objet) errors.push("Objet requis");
      if (!this.correspondance.date) errors.push("Date requise");
      if (errors.length > 0) {
    ...
    async submitCorrespondance() {
      if (!this.correspondance.reference || !this.correspondance.destinataire || !this.correspondance.objet || !this.correspondance.date) {
        alert('Veuillez remplir tous les champs requis');
        return;
      }
      console.log('Validation et enregistrement de la correspondance :', this.correspondance);
      ...
      try {
        ...
        if (response.ok) {
          alert('Correspondance externe validée et enregistrée avec succès !');
          this.resetForm();
          this.showAddModal = false;
          await this.fetchCorrespondances();
        } else {
          const errorData = await response.json();
          alert(`Erreur : ${errorData.error || 'Échec de l’enregistrement de la correspondance'}`);
        }
      } catch (error) {
        console.error('Erreur lors de l’enregistrement de la correspondance :', error);
        alert(`Erreur lors de l’enregistrement de la correspondance : ${error.message}`);
      }
    },
      ...
      try {
        ...
        reader.onload = async (e) => {
          ...
          const validCorrespondances = correspondances.filter(
            (c) => c.reference && c.destinataire && c.objet && c.date
          );
          ...
          try {
            const response = await fetch('http://localhost:3000/api/correspondances-externes/bulk', {
              method: 'POST',
    ...
    exportToExcel() {
      ...
      XLSX.write_file(workbook, 'correspondances_externes.xlsx');
    },
    printTable() {
      const printContent = `
        <html>
          <head>
            <title>Impression des Correspondances</title>
            <style>
          ...
          <body>
            <h2>Liste des Correspondances Externes</h2>
            <table>
              <thead>
                <tr>
                  <th>Référence</th>
    ...
    async deleteCorrespondance(id) {
      ...
      try {
        const response = await fetch(`http://localhost:3000/api/correspondances-externes/${id}`, {
          method: 'DELETE',
        });
        if (response.ok) {
    ...
    resetForm() {
      this.correspondance = { reference: '', destinataire: '', objet: '', date: '' };
      this.pieceJointe = null;
      this.$refs.fileInput.value = '';
    },
  },
